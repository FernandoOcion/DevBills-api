...
export class TransactionsRepository {
	...
	> {
		...
		const TransactionsMap = Transactions.map(
			(item) => item.toObject<Transaction>(),
		);
	...
	async getFinancialEvolution({
		year,
	}: GetFinancialEvolutionDTO): Promise<
		Balance[]
	> {
		const aggregate =
			this.model.aggregate<Balance>();
